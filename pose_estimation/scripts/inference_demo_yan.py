...
from write_keypoint_json import write_keypoint_json
...
def parse_args():
    ...
    parser.add_argument('--inferenceFps', type=int, required=False)
    parser.add_argument('--visthre', type=float, default=0)
    parser.add_argument('opts',
                        help='Modify config options using the command-line',
                        default=None,
                        nargs=argparse.REMAINDER)
    parser.add_argument('--save_im_vd', type=int, default=0, help='save image and video')
...
def main():
    ...
    open = vidcap.isOpened()
    fps = vidcap.get(cv2.CAP_PROP_FPS)
    if args.inferenceFps:
    ...
    while vidcap.isOpened():
        ...
        # write keypoint information into json file
        json_file_path = os.path.join(pose_dir, 'pose_{:012d}_keypoints.json'.format(count))
        write_keypoint_json(count, json_file_path, pose_preds, version=1.0)
        ...
        img_file = os.path.join(pose_dir, 'pose_{:012d}_keypoints.jpg'.format(count))  # todo save frame in /output/pose/

        if args.save_im_vd == 1:
            # draw skeleton
            image_skeleton = draw_skeleton(image_debug, pose_preds)
            cv2.imwrite(img_file, image_skeleton) # save image
            # outcap.write(image_debug)
            outcap.write(image_skeleton) # save video
    ...
    # csv_output_filename = os.path.join(args.outputDir, 'pose-data.csv')  # todo save pose_preds info into csv
